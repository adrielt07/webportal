...
resource "aws_ecs_service" "webapp" {
  name             = "${local.prefix}-webapp-service"
  cluster          = aws_ecs_cluster.main.name
  task_definition  = aws_ecs_task_definition.webapp.family
  desired_count    = 1
  launch_type      = "FARGATE"
  platform_version = "1.4.0" // pin this version of fargate

  // Notes Urgent - I need to change this to private,
  // after I spin up a Load balancer first
